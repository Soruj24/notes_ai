...
export default async function ArchivedPage() {
  const { user, workspace } = await requireWorkspace("/notes/archived");
  const [notes, tags] = await Promise.all([
    listUserNotes(user.id, workspace.id, { isArchived: true }),
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-end gap-2">
        ...
        <span
          aria-label={`${notes.length} archived notes`}
        ...
        <p className="w-full text-sm leading-6 text-zinc-500 dark:text-zinc-400">
          Out of the workspace, still searchable here.
        </p>
      </div>
      <NotesList
