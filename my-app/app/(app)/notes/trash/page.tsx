...
export default async function TrashPage() {
  const { user, workspace } = await requireWorkspace("/notes/trash");
  const [notes, tags] = await Promise.all([
    listUserNotes(user.id, workspace.id, { isDeleted: true }),
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-end gap-2">
        ...
        <span
          aria-label={`${notes.length} trashed notes`}
        ...
        <p className="w-full text-sm leading-6 text-zinc-500 dark:text-zinc-400">
          Deleted notes rest here until you restore or purge them.
        </p>
      </div>
      <NotesList
